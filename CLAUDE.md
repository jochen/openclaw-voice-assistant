...
- **`clawdpi`** — main Pi (clawdpi1, <speaches-host> area), DEVICE_INDEX=1, 48kHz mic resampled to 16kHz
...
| Service | URL | Purpose |
|---|---|---|
| Speaches (GPU container) | `http://<speaches-host>:8000` | STT (`/v1/audio/transcriptions`) + TTS (`/v1/audio/speech`) — OpenAI-compatible |
...
The `x-openclaw-session-key` header carries the session identifier (e.g. `agent:main:telegram:group:<telegram-group>`).
